...
class Genius(Game):
    ...
    def falar(self):
        ...
        for i in range(self.level):
            self.raffle.append(random.choice(self.color_options))
    ...
    def checking(self):
        ...
        for i in range(self.level):
            if self.sequence_user[i] == str(self.raffle[i]):
                print(f'Color {i + 1} is correct!')
            else:
                RED = "\033[1;31m"
                RESET = "\033[0;0m"
                print(RED + f'Color {i + 1} is incorrect. Correct color was {self.raffle[i]}' + RESET)
